...
function resolveCover(song: SongItem) {
  const cover = song.coverPath || song.coverDataUrl;
  if (!cover) return "";
...
function DetailPanel({ song }: { song: SongItem }) {
  ...
  return <aside className="card detail-panel">
    ...
    <div className="detail-cover">
      {cover ? <img src={cover} alt="cover" onError={(e)=>{
        console.error("[Cover Render Failed]", cover.slice(0,80));
...
export default function App(){
  ...
  const selectSong=async(song:SongItem)=>{
    ...
    try{
      console.log("[Detail Request]", song.path);
