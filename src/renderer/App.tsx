...
function DetailPanel({song,onClose}:{song:SongItem;onClose:()=>void}){
 ...
 return <aside className="card detail-panel">
  <div className="detail-header"><h2>歌曲详情</h2><button className="close-btn" onClick={onClose}>关闭</button></div>
 ...
 return <main className="app-container">
  ...
  <div className="music-layout">
   <section className="card song-card">
    ...
    <table><thead><tr><th>标题</th><th>艺术家</th><th>专辑</th><th>流派</th><th>歌词</th></tr></thead>
    <tbody>{filtered.map(song=><tr key={song.path} className={selectedSong?.path===song.path?"selected-row":""} onClick={()=>selectSong(song)}><td>{song.title}</td><td>{song.artist}</td><td>{song.album}</td><td>{song.genre||"-"}</td><td>{getLyricsLabel(song)}</td></tr>)}</tbody></table>
    </div>
   </section>
